...
while true {
    ...
    var sourceSyntax = Parser.parse(source: input)
    var diagnostics = Preprocessor.process(&sourceSyntax)
    diagnostics += ParseDiagnosticsGenerator.diagnostics(for: sourceSyntax)
